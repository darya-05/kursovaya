...
// Пустой массив, который в дальнейшем будет заполняться облаками
char a[9][9] = {
{'-', '2', '4', '2', '2', '0', '6', '6', '6'},
{'3', '-', '-', '-', '-', '-', '-', '-', '-'},
{'3', '-', '-', '-', '-', '-', '-', '-', '-'},
...
char itog[9][9]; // массив, в который будем считывать массив из файла
...
int pole();
int play();
int poleX(int x, int y, int clouddlin, int cloudshir);
int resetArray(char a[9][9]);
int savetofile(const char* filename, char mas[9][9]);
int savetofileProgress(const char* filename, char mas[9][9]);
int loadfromfile(char a[9][9]);
int checkCloud(int x, int y, int dlin, int shir);
int checkOverlap(char a[9][9], int x, int y, int dlin, int shir);
...
int play()
{
    ...
    savetofile("itog.txt", pole1); // Запись массива pole1 в файл itog.txt
    ...
    for (int i = 0; i < 9; i++) //Добавление облака на поле размером 9 на 9
    {
        ...
        do
        {
            ...
            if (checkCloud(x, y, dlin, shir) == 1 && checkOverlap(a, x, y, dlin, shir) == 1)
            {
...
int poleX(int x, int y, int dlin, int shir)
{
...
// Сравнение массива в файле с получившимся
int proverka(char a[9][9], char pole1[9][9])
{
